  ...
  <div class="page-wrapper">
    <div class="page-breadcrumb">
      <div class="col-5 align-self-center">
        <h4 class="page-title">UserOfRole</h4>
      </div>
    </div>
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">List UserOfRole
              </h4>
              <input
                  class="input-search"
                  placeholder="Search..."
            ...
            <div class="table-responsive">
              <table class="table table-hover">
                ...
                <tbody>
                <tr v-for="UserOfRole in list = ListUserOfRole" :key="UserOfRole.id">
                  <th scope="row">{{ UserOfRole.id }}</th>
                  <td>{{ UserOfRole.full_name }}</td>
                  <td>{{ UserOfRole.username }}</td>
                  <td>{{ UserOfRole.phone_number }}</td>
                  <td>{{ UserOfRole.address }}</td>
                  <td>
                    <router-link :to="`/view-user-of-role/${UserOfRole.id}/role/${$route.params.id}`"  tag="button">View</router-link>
                    <button
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      // this.checklogin();
      await axios
          .get('/admin/role/userRole/' + id+"?page="+this.paginate.current_page, token)
          .then(response => {
            console.log(response.data.data)
            this.all = response.data.data
            this.ListUserOfRole = this.all.data
    ...
    async deleteUser(e) {
      ...
      await axios.delete('users/' + id,token)
          .then(response => {
    ...
    async searchHandler()  {
      if (this.searchValue !== "") {
        ...
        // });
        const newUserOfRoleList = Object.values(this.ListUserOfRole).filter((userOfRole) => {
          console.log(userOfRole)
          return Object.values(userOfRole)
              .join(" ")
              .toLowerCase()
              .includes(this.searchValue.toLowerCase());
        });
        console.log(newUserOfRoleList)
        this.searchResults = newUserOfRoleList;
      } else {
        this.searchResults = this.ListUserOfRole;
      }
