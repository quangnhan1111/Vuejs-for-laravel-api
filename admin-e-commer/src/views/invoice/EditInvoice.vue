              ...
              <div class="form-group">
                <table class="table table-hover">
                  ...
                  <tr>
                    ...
                    <td>
                      <b-form-select v-model="Invoice.is_paid" :options="listStatus"></b-form-select>
                    </td>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <tr v-for="(element, index) in listInvoiceDetails" :key="index">
                      ...
                      <td>{{ element.name_size }}</td>
                      <td>{{ element.numberSoldOut }}</td>
                      <td>{{ element.TotalNumberWareProduct }}</td>
                      <td>{{ element.price * element.numberSoldOut }}.00$</td>
                    </tr>
                    </tbody>
                  </table>
                </div>
...
export default {
  ...
  methods: {
    async getData (){
      ...
      await axios.get('/invoice/showByCustomer/'+id, this.token).then((response)=> {
        console.log(response.data.data.data)
        this.listInvoiceDetails = response.data.data.data
        this.nameCustomerOrder = response.data.data.data[0].FullNameCustomer
        this.totalPriceForThisInvoice = response.data.data.data[0].TotalForPay
        this.is_paid = response.data.data.data[0].is_paid
      }).catch(() =>{
      });

      await axios.get('/invoice/showByCustomer/get/'+id, this.token).then((response)=> {
        console.log(response.data.data.data)
    ...
    async handleEdit() {
      ...
      let id = this.$route.params.id
      await axios.put('invoices/' + id, data, this.token).then((response) => {
