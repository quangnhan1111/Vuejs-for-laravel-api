    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">List Employee
                <router-link to="/add-employee"  tag="button">New</router-link>
              </h4>
              <input
                  class="input-search"
                  placeholder="Search..."
            ...
            <div class="table-responsive">
              <table class="table table-hover">
                ...
                <tbody>
                <tr v-for="Employee in list = searchValue.length < 1 ? ListEmployee : searchResults " :key="Employee.id">
                  ...
                  <td>
                    <router-link :to="`/edit-employee/${Employee.id}`"  tag="button">Edit</router-link>
                    <button
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      // this.checklogin();
      await axios
          .get('/admin/user/employee/?page='+this.paginate.current_page, token)
          .then(response => {
            console.log(response.data.data)
            this.all = response.data.data
            this.ListEmployee = this.all.data
    ...
    async searchHandler()  {
      if (this.searchValue !== "") {
        ...
        //   headers: {'Authorization': `Bearer ${localStorage.getItem("token")}`}
        // }
        // await  axios.get('admin/user/employee?search='+this.searchValue, token).then((response)=> {
        //   // console.log(response.data)
        //   this.li(response.data.content);
        // }).catch((error) =>{ console.log(error)
        // });
        const newEmployeeList = Object.values(this.ListEmployee).filter((employee) => {
          return Object.values(employee)
              .join(" ")
              .toLowerCase()
              .includes(this.searchValue.toLowerCase());
        });
        this.searchResults = newEmployeeList;
      } else {
        this.searchResults = this.ListEmployee;
      }
