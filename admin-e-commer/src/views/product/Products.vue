          ...
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">List Product
                ...
                <router-link to="/add-product"  tag="button">New</router-link>
                </template>
              </h4>
              <input
                  class="input-search"
            ...
            <div class="table-responsive">
              <table class="table table-hover">
                ...
                <tbody>
                <tr v-for="product in list = searchValue.length < 1 ? listProduct : searchResults " :key="product.id">
                  ...
                  <td><img :src="product.link" width="100px"  height="100px" alt=""></td>
                  <td>
                    <router-link :to="`/view-product/${product.id}`"  tag="button">View</router-link>
                    <template v-if="auth == 1">
                    <router-link :to="`/edit-product/${product.id}`"  tag="button">Edit</router-link>
                    <button
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      // this.checklogin();
      await axios
          .get('/productsForAdmin/?page='+this.paginate.current_page, token)
          .then(response => {
    ...
    async deleteProduct(e) {
      ...
      await axios.delete('products/' + id,token)
          .then(response => {
    ...
    async searchHandler()  {
      if (this.searchValue !== "") {
        ...
        //   headers: {'Authorization': `Bearer ${localStorage.getItem("token")}`}
        // }
        // await  axios.get('product?search='+this.searchValue, token).then((response)=> {
        //   // console.log(response.data)
        //   this.listProduct = response.data.content;
        // }).catch((error) =>{ console.log(error)
        // });
        const newProductList = Object.values(this.listProduct).filter((product) => {
          return Object.values(product)
              .join(" ")
              .toLowerCase()
              .includes(this.searchValue.toLowerCase());
        });
        this.searchResults = newProductList;
      } else {
